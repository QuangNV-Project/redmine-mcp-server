pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'quangnv1911/mcp-server'
                    ...
                    withCredentials([
                        ...
                        sh """
                            ssh -o StrictHostKeyChecking=no -i $SSH_KEY -p $REMOTE_PORT $REMOTE_USER@$REMOTE_HOST '
                                ...
                                docker run -d --name ${CONTAINER_NAME} --env-file ./infra/\$ENV_FILE --network prod-network -p \$PORT:3000 -v /logs/mcp-server:/app/logs --restart unless-stopped ${DOCKERHUB_IMAGE}:${IMAGE_TAG}
                            '
                        """
                    }
